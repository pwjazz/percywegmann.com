...
/*!
naturalDay
For dates that are the current day or within one day, return ÒtodayÓ, ÒtomorrowÓ or ÒyesterdayÓ, as appropriate. Otherwise, format the date using the passed in format string.

Argument: Date formatting string as described in the date tag.

Examples (when ÔtodayÕ is 17 Feb 2007):
...
For datetime values, returns a string representing how many seconds, minutes or hours ago it was Ð falling back to a longer date format if the value is more than a day old. In case the datetime value is in the future the return value will automatically use an appropriate phrase.

Examples (when ÔnowÕ is 17 Feb 2007 16:30:00):
...
truncatechars
New in Django 1.4: Please see the release notes
Truncates a string if it is longer than the specified number of characters. Truncated strings will end with a translatable ellipsis sequence ("É").
...
If value is "Joel is a slug", the output will be "Joel iÉ".
/**/
Humanize.truncatechars=function(a,b){if(a.length>b){return a.substr(0,b-3)+"É"}else{return a}};
/*!
truncatewords
...
If value is "Joel is a slug", the output will be "Joel is É".

Newlines within the string will be removed.
/**/
Humanize.truncatewords=function(b,d){var e=b.split(" ");var a="";for(var c=0;c<d;c++){if(e[c]===undefined){break}a+=e[c];a+=" "}if(e.length>d){a+="É"}return a};